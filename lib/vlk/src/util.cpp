...
std::string vlk::to_string(VkFormat f)
{
    switch (f) {
...
std::string vlk::to_string(VkColorSpaceKHR cs)
{
    switch (cs) {
...
std::string vlk::to_string(VkPresentModeKHR pm)
{
    switch (pm) {
    ...
    return std::to_string(static_cast<int>(pm));
}
...
void vlk::log(vlk::swap_properties_selection const& sps, std::string const& prefix)
{
    VLK_LOG_DEBUG() << prefix << "surface format: " << to_string(sps.surface_format.format) << " : " << to_string(sps.surface_format.colorSpace);
    VLK_LOG_DEBUG() << prefix << "presentation mode: " << to_string(sps.present_mode);
    VLK_LOG_DEBUG() << prefix << "extend: " << sps.extend.width << " x " << sps.extend.height;
    VLK_LOG_DEBUG() << prefix << "image count: " << sps.image_count;
    VLK_LOG_DEBUG() << prefix << "transform flags: 0x" << std::hex << sps.pre_transform << " composite_alpha flags: 0x" << std::hex << sps.composite_alpha;
}
